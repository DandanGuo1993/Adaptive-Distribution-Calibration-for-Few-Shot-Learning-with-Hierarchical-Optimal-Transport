...
def GenerateRun(iRun, cfg, regenRState=False, generate=True):
    ...
    if generate:
        ...
        dataset_mean = torch.zeros((cfg['ways'],  data.shape[2]))
    ...
    for i in range(cfg['ways']):
        ...
        if generate:
            ...
            dataset_mean[i] = torch.mean(data[classes[i], :, :], dim=0, keepdim=False)
    return dataset,classes,dataset_mean
...
def setRandomStates(cfg):
    ...
    rsFile ="./cache/RandStates_miniImagenet_s1_q15_w5"
...
def GenerateRunSet(start=None, end=None, cfg=None):
    ...
    classindex = np.zeros((end-start, cfg['ways']),dtype=int)
    for iRun in range(end-start):
        dataset[iRun],classindex[iRun],dataset_mean = GenerateRun(start+iRun, cfg)
    return dataset,classindex,dataset_mean
